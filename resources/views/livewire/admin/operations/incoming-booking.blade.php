<div class="admin-layout" wire:poll.10s>
    ...
    <main class="main-content">
        ...
        <div class="content-area">
            <div class="mx-auto w-full max-w-7xl px-6 py-6 space-y-6">
                ...
                <flux:card class="p-4">
                    <div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
                        <div class="flex-1 max-w-md">
                            <flux:input wire:model.live.debounce.300ms="search" placeholder="Cari kode sewa, nama pelanggan, NIK, atau WhatsApp..." icon="magnifying-glass" />
                        </div>
                        <!-- Status Filter Tabs -->
                        <div class="flex items-center gap-1.5 bg-slate-100 p-1 rounded-xl shrink-0">
                            <button type="button" wire:click="$set('filterStatus', 'all')" class="px-3 py-1.5 rounded-lg text-xs font-bold transition {{ $filterStatus === 'all' ? 'bg-white text-navy shadow-xs' : 'text-slate-500 hover:text-navy' }}">Semua</button>
                            <button type="button" wire:click="$set('filterStatus', 'dp')" class="px-3 py-1.5 rounded-lg text-xs font-bold transition {{ $filterStatus === 'dp' ? 'bg-white text-sky-600 shadow-xs' : 'text-slate-500 hover:text-sky-600' }}">DP Terbayar</button>
                            <button type="button" wire:click="$set('filterStatus', 'paid')" class="px-3 py-1.5 rounded-lg text-xs font-bold transition {{ $filterStatus === 'paid' ? 'bg-white text-emerald-600 shadow-xs' : 'text-slate-500 hover:text-emerald-600' }}">Lunas</button>
                            <button type="button" wire:click="$set('filterStatus', 'active')" class="px-3 py-1.5 rounded-lg text-xs font-bold transition {{ $filterStatus === 'active' ? 'bg-white text-emerald-600 shadow-xs' : 'text-slate-500 hover:text-emerald-600' }}">Menunggu Diambil</button>
                            <button type="button" wire:click="$set('filterStatus', 'expired')" class="px-3 py-1.5 rounded-lg text-xs font-bold transition {{ $filterStatus === 'expired' ? 'bg-white text-rose-600 shadow-xs' : 'text-slate-500 hover:text-rose-600' }}">Lewat Waktu</button>
                        </div>
                    </div>
                </flux:card>
                ...
                @if(session()->has('message'))
                    <div class="p-4 bg-emerald-50 border border-emerald-200 text-emerald-800 text-sm font-bold rounded-2xl flex items-center gap-2.5 shadow-xs">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-emerald-600 shrink-0" viewBox="0 0 20 20" fill="currentColor">
                ...
                @if(session()->has('error'))
                    <div class="p-4 bg-red-50 border border-red-200 text-red-800 text-sm font-bold rounded-2xl flex items-center gap-2.5 shadow-xs">
                ...
                <div class="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
                    @forelse($bookings as $booking)
                        <flux:card class="p-0 overflow-hidden flex flex-col group relative transition-all duration-200 {{ $booking->is_expired ? 'border-rose-200 shadow-sm ring-1 ring-rose-100' : 'border-slate-200/90 shadow-xs hover:shadow-md' }}" x-data="{ expanded: false }">
                            
                            <!-- Card Header -->
                            <div class="p-5 border-b border-slate-100 {{ $booking->is_expired ? 'bg-rose-50/30' : 'bg-slate-50/40' }}">
                                <!-- Top Meta Row -->
                                <div class="flex items-center justify-between gap-2 mb-3">
                                    ...
                                    <flux:badge size="sm" color="amber" icon="archive-box">
                                        {{ $booking->details->count() }} Unit
                                    </flux:badge>
                                </div>
                                ...
                                <div class="mt-4 rounded-xl border {{ $booking->is_expired ? 'border-rose-200/90 bg-rose-50/40' : 'border-slate-200/80 bg-slate-50/70' }} overflow-hidden">
                                    ...
                                    @if($booking->is_expired)
                                        <div class="px-3 py-2 bg-rose-100/90 border-t border-rose-200/90 text-rose-900 text-xs flex items-center gap-2">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 shrink-0 text-rose-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    ...
                                    @else
                                        <div class="px-3 py-2 bg-emerald-100/70 border-t border-emerald-200/80 text-emerald-900 text-xs flex items-center gap-2">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 shrink-0 text-emerald-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                                            </svg>
                                ...
                                <div class="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between">
                                    <div>
                                        ...
                                        @if($booking->status === 'DP_PAID')
                                            <div class="text-[11px] font-bold text-sky-700 mt-0.5">
                                                DP: Rp {{ number_format($booking->down_payment_amount, 0, ',', '.') }} 
                                                <span class="text-amber-700 font-extrabold ml-1">(Sisa: Rp {{ number_format($booking->balance_due, 0, ',', '.') }})</span>
                                            </div>
                                        @elseif($booking->status === 'PAID')
                                    ...
                                    <button type="button" @click="expanded = !expanded" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold transition-colors cursor-pointer select-none">
                                        ...
                                        <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5 transform transition-transform duration-200 text-slate-500" :class="{'rotate-180': expanded}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <!-- Modal Konfirmasi Pembatalan Booking (Standard Flux Modal - Perfectly Centered) -->
        <flux:modal wire:model="showCancelModal" class="max-w-lg">
            <div class="space-y-6">
                <div class="flex items-start gap-4">
                    ...
                    <div class="flex-1 min-w-0">
                        <flux:heading size="lg">Konfirmasi Pembatalan Booking</flux:heading>
                ...
                <div class="flex items-center justify-end gap-3 pt-2">
                    <flux:button type="button" wire:click="closeCancelModal" variant="ghost">
                        Kembali
                    </flux:button>
                    <flux:button type="button" wire:click="executeCancellation" variant="danger" wire:loading.attr="disabled">
